...
Read-only. Writes screenshots/c3_survivor_{n}.gif
"""
...
def main() -> int:
    ...
    cands = [(int(r[0]), legs.get(r[1])) for r in s.execute(
        "SELECT vehicle_track_id, origin_leg_id FROM vehicle_events "
        "WHERE camera_id=? AND rejected=0 AND movement='u_turn' AND "
        "posterior_source='gate_full' ORDER BY classifier_path_distance "
        "DESC LIMIT 10", (CAM,))]
    s.close()
    ...
    for tid, ocard in cands:
        ...
        if not after:
            continue
        f_out = int(after[0][0])
        if f_out - f_in > 60 * fps:          # keep the clip watchable
            continue
        ...
        if len(frames) < 8:
            continue
        made += 1
        out = Path(f"screenshots/c3_survivor_{made}.gif")
        ...
        print(f"clip {made}: tid {tid} {ocard}->{ocard} u_turn, in@{f_in} "
