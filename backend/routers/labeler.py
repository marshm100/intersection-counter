...
def labeler_save(ds: str, payload: LabelPayload):
    ...
    for b in payload.boxes:
        ...
        cls = int(b[0])
        # 3 = "medium" (round v2: buses + non-long single-unit trucks — the
        # Miovision Mediums bucket; see prep_finetune_labels.NAMES).
        if cls not in (0, 1, 2, 3):
            raise HTTPException(status_code=422, detail="class must be 0-3")
