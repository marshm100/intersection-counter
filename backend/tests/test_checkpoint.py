...
class TestCheckpointManager:
    ...
    def test_save_and_load(self, mgr):
        ...
        assert cp["pedestrian_count"] == 2
        assert cp["error_count"] == 1

    def test_upsert_overwrites(self, mgr):
        mgr.save_checkpoint(100, 3.33, b"a", b"b", 1, 0, 0)
        mgr.save_checkpoint(200, 6.66, b"c", b"d", 10, 5, 2)
        cp = mgr.load_checkpoint()
    ...
    def test_tracker_state_blob(self, mgr):
        ...
        mgr.save_checkpoint(50, 1.5, blob, b"", 0, 0, 0)
        cp = mgr.load_checkpoint()
