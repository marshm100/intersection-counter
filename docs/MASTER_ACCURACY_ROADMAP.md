...
| QD | **Discharge-event counting — FAILED G-QD-1 (2026-08-19), iteration 1 DEAD.** Operator bbox review: the "eventless fulls" are ID-SWITCH SPLICES (track rides car A to the horizon, then locks onto opposite-direction car B — the join fakes a full journey). The pipeline's rejects were CORRECT; recovery-by-policy on unsplit tracks is dead. STANDING HAZARD recorded: approach-bar gains from additive candidates in deficit regimes are NOT evidence (garbage helps deficits) — human/identity review is mandatory for additive candidates. Survivors: funnel instrument, guards, the bbox review page | **DEAD at the human gate**; gate doc has the full verdict | — | — | closed |
...
- 2026-08-19: QD iteration 1 FAILED at the human gate — the
  recoverable class is ID-SWITCH SPLICES (operator bbox review,
  17/17). A3 splice-split promoted to the #1 volume-chain lever;
  approach-bar additive-gain blind spot recorded as a standing
  hazard; E4R's "recoverable" framing corrected in place.
