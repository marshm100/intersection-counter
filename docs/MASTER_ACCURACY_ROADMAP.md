...
| QD | **Discharge-event counting** — PLANNED with pre-declared gates: docs/plan_qd_discharge_counting_2026-08-18.md. Funnel MEASURED at cam2-1600: 565 eventless exit-crossers = 280 pure pipeline drops (median 8.7 s tracks; ~half the window volume gap) + 285 rejected-row (iteration 2, dedup re-adjudication via E1). Goes through the ORIGINAL apply gate (adds events — zero-mass does not hold) | plan doc committed; build after the auto-cal frees the repo | +0/+2/+5 | +0/+3/+7 | days |
